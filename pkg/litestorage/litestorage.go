...
func (s *LiteStorage) SearchTxHashInStorage(hash tongo.Bits256) (tongo.Bits256, error) {
	...
	if err != nil {
		s.logger.Info("Transaction hash not found",
			zap.String("search_hash", hash.Hex()),
			zap.Error(err))
...
func (s *LiteStorage) GetMasterchainTransactions(ctx context.Context, masterchainSeqno int32) ([]core.Transaction, error) {
	...
	default:
		...
		s.logger.Info("shardIDs", zap.Any("shardIDs", shardIDs))
		if err != nil {
		...
		for _, id := range shardIDs {
			...
			default:
				...
				// Move logging to debug level and only log if needed
				if len(shardTxs) > 0 {
					s.logger.Debug("processing shard transactions",
						zap.Any("id", id),
						zap.Int("transaction_count", len(shardTxs)))
				}
